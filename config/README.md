...
On the 4L/256H P4 probe, this preserved the baseline loss curve while damping about 36% of probed anti-aligned update elements. On the tuned 12L/1024H looped-4 interleaved P4 comparison, it slightly improved every no-MoD validation checkpoint and finished at val loss `4.887799` vs control `4.890851` (`-0.003052`, about `0.31%` lower perplexity) with matched train loss and throughput. A fresh 2000-step extension of the same no-MoD + filter schedule continued descending to val loss `4.056977` / ppl `57.80`, with monotonic 200-step evals and mean keep fraction `0.5722`; this was `-0.830822` val loss and `56.4%` lower perplexity than the earlier 600-step no-MoD + filter result. The no-renorm variant is useful as an ablation, but it slowed both train and validation loss by shrinking the effective step. MoD should be tuned separately before combining it with this default; the filter recovered part of MoD's validation penalty but did not make MoD competitive with the no-MoD arms on the 600-step slice.
