...
- ✅ ALiBi (linear attention bias, no params, flash-native `alibi_slopes`) — `src/models/position/alibi.py`; `pos_type = "alibi"`
- ✅ Relative Position Bias (learned bucket bias, T5-style) — `src/models/position/rel_pos_bias.py`; `pos_type = "rel_pos"`, `rel_pos_num_buckets`
- ✅ `pos_type` config field dispatches all 5 variants; back-compat for `rope_base`-only configs
