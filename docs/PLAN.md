...
- ☐ **Architecture/code memory optimizations before the next shape sweep**:
	...
	- Add generalized looped attention / looped block execution so Phase 5 can keep logical depth while reusing a smaller physical block stack. Durable plan: [LOOPED_ATTENTION_PLAN.md](LOOPED_ATTENTION_PLAN.md). First implementation slice should generalize `share_layer_weights` into `looped_num_blocks`, preserve per-logical-layer KV caches, then smoke 12 logical MLA+block_attn layers against 6/4 physical-block variants.
