...
Phase 3 continued. FeedForward, TransformerBlock, and DecoderLM are implemented with full unit + integration tests. Model factory now supports DecoderLM, and a DecoderLM validation config is in place. README/DESIGN diagrams were updated to reflect KV-cache + DPO flow and standardized diagram layouts. 373 tests passing; lint/mypy/black clean.
...
⏭️ **Phase 3 continues**: Training loop upgrades (LR scheduler, AMP, gradient accumulation) — start with simple LR warmup + cosine decay, then gradient clipping + mixed precision
